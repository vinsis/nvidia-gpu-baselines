...
with torch.backends.cuda.sdp_kernel(
    ...
    print('Enable math took {} seconds for {} trials'.format(end - start, num_trials))
...
with torch.backends.cuda.sdp_kernel(
    ...
    start = time.time()
    for i in range(num_trials):
        # non-zero dropout not supported by memory efficient attention
        out = F.scaled_dot_product_attention(q, k, v, dropout_p=0)
    torch.cuda.synchronize()
    end = time.time()
    print('Memory efficient took {} seconds for {} trials'.format(end - start, num_trials))
